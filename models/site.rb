class Site
  ...
  def protestor_users
    set = protestors
    visits = set.map{|uuid| Visit.first(:site_id => self.id, :uuid => uuid) }.compact
  ...
  def add_protestor(uuid, opts={})
    # TODO use time-based expiry and/or sorted-sets
    # so we only get protestors who were active in the last N minutes
    puts "add_protestor =========> uuid=#{uuid.inspect} opts=#{opts.inspect}"
    redis.sadd(protestors_key, uuid)
  end
